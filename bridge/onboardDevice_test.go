...
func TestOnboardDevice(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			defer ttCancel()
			err = c.OnboardDevice(ttCtx, tt.args.deviceID, tt.args.authorizationProvider, tt.args.cloudURL, tt.args.authorizationCode, tt.args.cloudID, client.WithDeviceID(d.GetID().String()))
			if tt.wantErr {
				require.Error(t, err)
				return
			}
			require.NoError(t, err)
			err = c.OffboardDevice(ttCtx, tt.args.deviceID, client.WithDeviceID(d.GetID().String()))
			require.NoError(t, err)
		})
	}
}
