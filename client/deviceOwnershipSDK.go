...
func newDeviceOwnershipSDK(app ApplicationCallback, sdkDeviceID string, dialTLS core.DialTLS,
	dialDTLS core.DialDTLS, signerCert *tls.Certificate, validFrom string, certExpiry time.Duration, crlDistributionPoints []string, createSigner func(caCert []*x509.Certificate, caKey crypto.PrivateKey, validNotBefore, validNotAfter time.Time, crlDistributionPoints []string) core.CertificateSigner,
) (*deviceOwnershipSDK, error) {
	if validFrom == "" {
		validFrom = "now-1m"
	}
	...
	return &deviceOwnershipSDK{
		...
		createIdentitySigner: func() (core.CertificateSigner, error) {
			...
			return createSigner(signerCAs, signerCert.PrivateKey, notBefore, notAfter, crlDistributionPoints), nil
		},
		app:      app,
		dialTLS:  dialTLS,
		dialDTLS: dialDTLS,
