...
func iotivityHack(ctx context.Context, tlsClient *coapClient, sdkID string) error {
	...
	err := tlsClient.UpdateResourceVNDOCFCBOR(ctx, "/oic/sec/doxm", setDeviceOwner, nil)
	if err != nil {
	...
	err = tlsClient.UpdateResourceVNDOCFCBOR(ctx, "/oic/sec/cred", iotivityHackCredential, nil)
	if err != nil {
		return fmt.Errorf("cannot set iotivity-hack credential: %v", err)
	}

	err = tlsClient.DeleteResourceVNDOCFCBOR(ctx, "/oic/sec/cred", nil, WithCredentialSubject(hackId))
	if err != nil {
...
func (c *Client) OwnDevice(
	...
	/*doxm doesn't send any content for select OTM*/
	err = client.UpdateResourceVNDOCFCBOR(ctx, "/oic/sec/doxm", selectOTM, nil)
	if err != nil {
		if ownership.Owned {
	...
	err = tlsClient.GetResourceVNDOCFCBOR(ctx, "/oic/sec/pstat", &provisionState)
	if err != nil {
	...
	err = tlsClient.UpdateResourceVNDOCFCBOR(ctx, "/oic/sec/pstat", updateProvisionState, nil)
	if err != nil {
	...
	err = tlsClient.GetResourceVNDOCFCBOR(ctx, "/oic/sec/csr", &csr)
	if err != nil {
	...
	err = tlsClient.GetResourceVNDOCFCBOR(ctx, "/oic/sec/cred", &deviceCredential, WithCredentialSubject(deviceID))
	if err != nil {
	...
	for _, cred := range deviceCredential.Credentials {
		...
		case cred.Usage == schema.CredentialUsage_CERT && cred.Type == schema.CredentialType_ASYMMETRIC_SIGNING_WITH_CERTIFICATE,
			...
			err = tlsClient.DeleteResourceVNDOCFCBOR(ctx, "/oic/sec/cred", nil, WithCredentialId(cred.ID))
			if err != nil {
	...
	err = tlsClient.UpdateResourceVNDOCFCBOR(ctx, "/oic/sec/cred", setIdentityDeviceCredential, nil)
	if err != nil {
	...
	for _, ca := range cas {
		...
		err = tlsClient.UpdateResourceVNDOCFCBOR(ctx, "/oic/sec/cred", setCaCredential, nil)
		if err != nil {
	...
	err = tlsClient.GetResourceVNDOCFCBOR(ctx, "/oic/sec/doxm", &verifyOwner)
	if err != nil {
	...
	/*doxm doesn't send any content for select OTM*/
	err = tlsClient.UpdateResourceVNDOCFCBOR(ctx, "/oic/sec/doxm", setDeviceOwned, nil)
	if err != nil {
	...
	err = c.UpdateResourceVNDOCFCBOR(ctx, deviceID, "/oic/sec/pstat", setOwnerProvisionState, nil)
	if err != nil {
	...
	err = c.UpdateResourceVNDOCFCBOR(ctx, deviceID, "/oic/sec/acl2", setOwnerAcl, nil)
	if err != nil {
	...
	err = c.UpdateResourceVNDOCFCBOR(ctx, deviceID, "/oic/sec/pstat", setProvisionStateToRFPRO, nil)
	if err != nil {
	...
	err = c.UpdateResourceVNDOCFCBOR(ctx, deviceID, "/oic/sec/pstat", setProvisionStateToRFNOP, nil)
	if err != nil {
