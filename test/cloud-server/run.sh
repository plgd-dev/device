...
/usr/local/bin/device.client.core.test $@ -test.coverprofile=/tmp/device.client.test.coverage2.txt

if [ ! -z "$coverprofile" ]; then
  cp /tmp/device.client.test.coverage1.txt $coverprofile
  tail -n +2 /tmp/device.client.test.coverage2.txt >> $coverprofile
fi